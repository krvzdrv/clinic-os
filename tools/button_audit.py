...
def scenario_anamnesis_exam_forms(page, audit: Audit):
    ...
    if _open_details(page, "Состояние") or _open_details(page, "Общее состояние"):
        sel = page.locator("form.gc-choice select[name='key'], .gc-choice select[name='key']").first
        btn = page.locator(".gc-choice button[type='submit'], form[action*='general_condition'] button[type='submit']").first
        if sel.count():
            _fill_select_first_real(page, ".gc-choice select[name='key']")
        if btn.count():
            btn.click()
            page.wait_for_timeout(600)
