...
# P1 / ДемоА: эталонный амбулаторный, нетяжёлая, амоксициллин, всё по протоколу.
def p1():
    pid = _patient("ДемоА", "Антон", "Петрович", "male", "1985-03-12")
...
# P3 / ДемоБ: неверная АБТ — азитромицин вместо амоксициллина (not_first_line_abt).
def p3():
    pid = _patient("ДемоБ", "Борис", "Иванович", "male", "1978-06-05")
    ...
    _obs(pid, "9279-1", "ЧД", 22, "/мин", -1, eid)
    _obs(pid, "6690-2", "Лейкоциты", 9.5, "10^9/L", -1, eid)
    _obs(pid, "30522-7", "СРБ", 42, "mg/L", -1, eid)
    _sr(pid, "CBC", "ОАК", -1, eid, status="completed")
    _sr(pid, "CRP", "СРБ", -1, eid, status="completed")
    _med(pid, "J01FA10", "Азитромицин", "oral", -1, 9, eid, dose="500 мг", freq="1 раз в день")
    cps.create_cap_plan(pid)
...
# P5 / ДемоВ: тяжёлая (SpO2 87, ЧД 32, САД 85), амбулаторно, АБТ не назначена → госпитализация.
def p5():
    pid = _patient("ДемоВ", "Виктор", "Сергеевич", "male", "1975-04-18")
