...
def quality_measure_cap():
    """
    Доля пациентов с активным протоколом (ВП / ЖДА / …), у которых primary-
    протокол в кэше выполняется (нет warning-отклонений).

    Берёт cap_cache (заполняется refresh_protocol_cache при записи/открытии
    карты), чтобы дашборд не делал N+1 evaluate_*. Имя функции историческое.
    """
