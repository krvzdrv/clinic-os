...
Блок UI «По протоколу» при `applicable=false` не показывается: без диагноза
протокол ещё не выбран (мультипротокол). Карточка «Диагноз не установлен» —
нейтральная подсказка + форма МКБ.
