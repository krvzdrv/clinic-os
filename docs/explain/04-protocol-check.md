...
1. Проверка происходит не по расписанию и не по кнопке — система пересчитывает картину сразу после любой записи, которая может повлиять на протокол: новый показатель, новый диагноз, новое назначение, новый результат обследования.
   Эта же логика — данные → сравнение с протоколом → замечания → подсказка — одинаково работает для любого из протоколов (сейчас 2: пневмония, анемия). Если у пациента активны оба диагноза одновременно, каждый проверяется отдельно и получает свою подсказку, а не общую на двоих.
