...
2. **Врач может принять своё решение.** Soft-stop и hard-stop на `order-sign` **не запрещают** назначение навсегда: требуется явный акт согласия (чекбокс или обязательная причина).
...
|---------|--------|-----|------------|
| **SOFT-STOP** (`severity=warning`) | АБТ не по протоколу (`not_first_line_abt`), неоптимальная осторожность drug_service | Чекбокс согласия; причина текстом **опциональна** | `confirm=1`, `ack=1`, `override_reason?` |
| **HARD-STOP** (`severity=hard-stop`) | Аллергия, противопоказание, опасное взаимодействие | Обязательная текстовая причина; одного чекбокса недостаточно | `confirm=1`, `override_reason` (непустой) |
...
| Soft: «Назначить всё равно» после чекбокса | Сохраняет с `cds_override=1`; пишет `cds_override_log` (`severity=soft-stop`). |
